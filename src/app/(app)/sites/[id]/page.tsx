...
type Tab = "opportunities" | "pages" | "clusters" | "install";
    ...
    <div className="pt-8">
      ...
      {status === "ready" && (
        <>
          ...
          <div className="mt-8 flex gap-1 border-b border-line">
            {(
              ...
              <button
                key={key}
                onClick={() => setTab(key)}
                className={`px-3.5 py-2.5 text-sm font-medium transition-colors border-b -mb-px ${
          ...
          <div className="pt-6">
            {tab === "opportunities" && (
              <Opportunities siteId={site.id} opps={opportunities} pageById={pageById} reload={load} />
            )}
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.slice(0, 200).map((o) => (
            <OppCard key={o.id} opp={o} pageById={pageById} onStatus={setStatus} />
          ))}
          {filtered.length > 200 && (
